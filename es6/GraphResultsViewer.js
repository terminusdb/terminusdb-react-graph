...
function GraphResultsViewer(config, result) {
  ...
  this.link_elements;
  this.node_elements; // DOM object that this this is directly drawn into

  this.container = false; // Map of ids to loaded nodes for ease

  this.loadedNodes = {};
  this.focusNodes = []; // Currently displayed nodes and links

  this.nodes;
  this.links; // Var for selecting and deselecting elements
...
GraphResultsViewer.prototype.loadNewData = function () {
  ...
  this.links = this.result.getEdges(); //this.browser.rebuildController();
};
/*
...
GraphResultsViewer.prototype.clear = function () {
  ...
  this.loadedNodes = {};
  this.links = [];
  this.updateGraph(); //jQuery(this.d3DOM).empty();
...
GraphResultsViewer.prototype.initD3 = function (jqid) {
  ...
  try {
    ...
    d3.select(jqid).selectAll("svg").remove(); //d3.select(".d3-component").select("svg").remove();
  } catch (err) {
    console.log('no svg');
  }
  /********************* Seed the data **************************/
  ...
  var distanceFun = function distanceFun(link) {
    return self.getLinkDistance(link);
  }; // create the force object


  this.simulation = d3.forceSimulation() //link distance varies with link type
  .force("link", d3.forceLink().distance(distanceFun).id(function (link) {
    return link.id;
  })) //charge is the same for all nodes
  .force('charge', d3.forceManyBody().strength(function (node) {
    return self.getCharge(node);
  ...
  })).force("center", d3.forceCenter(this.width / 2, this.height / 2)); // node drag

  this.drag_drop = d3.drag().on('start', function (node) {
    node.fx = node.x;
    node.fy = node.y;
  ...
  this.svg.append("rect").attr("width", this.width).attr("height", this.height).style("fill", "none").style("pointer-events", "all").call(d3.zoom().on("zoom", function () {
    self.zoomed();
...
GraphResultsViewer.prototype.zoomed = function () {
  ...
  if (this.node_elements) this.node_elements.attr("transform", d3.event.transform);
  if (this.link_elements) this.link_elements.attr("transform", d3.event.transform);
};
/***
...
GraphResultsViewer.prototype.updateGraph = function (nodes, links) {
  ...
  var follow_node_wrapper = function follow_node_wrapper(graph_node) {
  ...
  var node_enter = this.node_elements.enter().append("g"); // enter and create new ones

  node_enter.append("circle").style('opacity', 0.99).attr('r', function (node) {
    return self.getRadius(node);
  }).attr("cx", function (node) {
    return node.x;
  }).attr("cy", function (node) {
    return node.y;
  }).attr("type", function (node) {
    return self.isFringe(node) ? "fringe" : "node";
  }).call(self.drag_drop).on("mouseover", function (d) {
    d3.select(this).style("cursor", "pointer");
  }).on("mouseout", function (d) {
    d3.select(this).style("cursor", "default");
  }).on('click', follow_node_wrapper);
  node_enter.append("text").attr("x", function (node) {
    return node.x;
  }).attr("y", function (node) {
    return node.y;
  }).text(function (node) {
    return self.getNodeIconUnicode(node);
  }).call(self.drag_drop).on("mouseover", function (d) {
    d3.select(this).style("cursor", "pointer");
  }).on("mouseout", function (d) {
    d3.select(this).style("cursor", "default");
  }).on('click', follow_node_wrapper); // merge new and old nodes

  this.node_elements = node_enter.merge(this.node_elements);
  this.styleNodeElements(); // update LINKS ********************************************

  this.link_elements = this.link_group.selectAll("line").data(links); // remove old links

  this.link_elements.exit().remove(); // enter and create new ones

  var link_enter = this.link_elements.enter().append("line"); // merge new and old links

  this.link_elements = link_enter.merge(this.link_elements);
  this.styleLinkElements(); //execute the current state of the pan-zoom transform

  if (this.current_transform) {
    if (this.node_elements) this.node_elements.attr("transform", this.current_transform);
    if (this.link_elements) this.link_elements.attr("transform", this.current_transform);
  }
};

GraphResultsViewer.prototype.updateSimulation = function (mode) {
  var self = this; //move all of the graphical elements corresponding to the nodes with each tick

  var ticker = function ticker() {
    self.node_elements.selectAll("circle").attr("cx", function (node) {
      return node.x;
    }).attr("cy", function (node) {
      return node.y;
    });
    self.node_elements.selectAll("text").attr("x", function (node) {
      return node.x;
    }).attr("y", function (node) {
      return node.y;
    });
    self.link_elements.attr('x1', function (link) {
...
GraphResultsViewer.prototype.styleNodeElements = function () {
  ...
  if (this.node_elements) {
    this.node_elements.each(function (node) {
      ...
      sel.classed("highlighted", function (node) {
        return self.focusNodes.indexOf(node.id) != -1;
      });
      sel.append("title").classed("terminus-gnode-title", true).text(self.getNodeText(node));
      var txt = g.select("text");
      txt.select("title").remove();
      txt.attr('text-anchor', 'middle') //.attr('class','labelClassName')
      .attr("title", self.getNodeText(node)).attr('dominant-baseline', 'central').style('font-family', "'" + self.fontfam + "'").style('font-weight', self.getNodeIconWeight(node)).style('font-size', self.getNodeIconSize(node)).style("fill", self.getNodeIconColour(node)).text(self.getNodeIconUnicode(node)) //"\uf061")
      .append("title").classed("terminus-gnode-title", true).text(self.getNodeText(node));
    });
  }
};
...
GraphResultsViewer.prototype.styleLinkElements = function () {
  ...
  if (this.link_elements) {
    this.link_elements.each(function (link) {
      ...
      sel.append("title").classed("terminus-glink-title", true).text(self.getLinkText(link));
    });
  }
};
...
GraphResultsViewer.prototype.getEdgeArrow = function (edge) {
  if (edge) {
    var dir = this.getEdgeDirection(edge); //if(dir){

    var col = this.getEdgeColour(edge);
    var reference;
    this.svg.append("svg:defs").selectAll("marker").data([reference]).enter().append("svg:marker").attr("id", String).attr("viewBox", "0 -5 10 10").attr("refX", 22) // This sets how far back it sits, kinda
    .attr("refY", 0).attr("markerWidth", this.getArrowWidth(edge)).attr("markerHeight", this.getArrowHeight(edge)).attr("orient", "auto").attr("markerUnits", "userSpaceOnUse").append("svg:path").attr("d", dir).style("fill", col);
...
GraphResultsViewer.prototype.nodeSelected = function (selected_node) {
  ...
  if (this.selected_id == selected_node.id && this.selection_grows) {
    ...
    this.styleNodeElements(); //this.browser.nodeSelected(selected_node);

    if (this.isFringe(selected_node)) {//this.browser.followNode(selected_node);
    }
  }
};
...
GraphResultsViewer.prototype.scale = function (min_ratio) {
  ...
  var x_trans = this.width - new_mol_width; // do the actual moving

  var self = this;
  if (this.node_elements) this.node_elements.transition().duration(750).attr("transform", function (node) {
    return self.getScaleTransform(x_trans, y_trans, min_ratio);
  });
  if (this.link_elements) this.link_elements.transition().duration(750).attr("transform", function (node) {
    return self.getScaleTransform(x_trans, y_trans, min_ratio);
  });
  this.scale_factor = min_ratio;
  this.svg.selectAll("marker").attr("markerWidth", this.getArrowWidth()).attr("markerHeight", this.getArrowHeight());
};
...
GraphResultsViewer.prototype.scaleToFit = function () {
  ...
  if (this.nodes.length === 0) return; // Get the bounding box

  min_x = d3.min(this.nodes.map(function (d) {
    return d.x;
  }));
  min_y = d3.min(this.nodes.map(function (d) {
    return d.y;
  }));
  max_x = d3.max(this.nodes.map(function (d) {
    return d.x;
  }));
  max_y = d3.max(this.nodes.map(function (d) {
    return d.y;
  ...
  mol_height = mol_height * 1.05; // how much larger the drawing area is than the width and the height

  width_ratio = this.width / mol_width;
  height_ratio = this.height / mol_height; // we need to fit it in both directions, so we scale according to
...
GraphResultsViewer.prototype.recentre = function (graph_node) {
  this.translated = true; // get the x and y you need to transform the nodes by

  this.dcx = this.width / 2 - graph_node.x;
  this.dcy = this.height / 2 - graph_node.y; //zoom.translate([dcx,dcy]); If we do zooming we'll need to do something here
...
GraphResultsViewer.prototype.setConfigOptions = function (config) {
  ...
  this.defaults = {
    edge: {
      ...
      arrow: config && config.edge && config.edge.arrow ? config.edge.arrow : {
        width: 20,
        height: 10
      },
      //{ width: 36, height: 16}),
      symmetric: config && config.edge && config.edge.symmetric ? config.edge.symmetric : true,
      color: config && config.edge && config.edge.color ? config.edge.color : [120, 118, 116],
      //[150,150,255]),
    ...
    node: {
      type: "node",
      radius: config && config.node && config.node.radius ? config.node.radius : 16,
      //14),
      ...
      color: config && config.node && config.node.color ? config.node.color : [245, 114, 7],
      //[245, 179, 66]),//[0,255,255]),
      icon: {
...
GraphResultsViewer.prototype.getNodeIconUnicode = function (node) {
  if (node && typeof node.icon != "undefined") {
    ...
    if (node.icon.label === true) return this.getNodeText(node);else if (node.icon.label) return node.icon.label;
  }
...
var _default = GraphResultsViewer;
